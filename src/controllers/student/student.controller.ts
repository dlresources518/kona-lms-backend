...
export default class StudentController {
  ...
  public createOrder = async (req: Request, res: Response) => {
    try {
      ...
      let awsResponse: any = "";
      let invoice: any;
      const orderDetails = await orderModel.find({studentId: req['tokenId'], courseId: req.body.courseId});
      if (orderDetails && orderDetails.length) {
        res.status(409).json({status: 'Course already mapped to this student'});
      } else {
        const studentDetails: any = await StudentModel.findById(req["tokenId"], {
          password: 0,
        });
