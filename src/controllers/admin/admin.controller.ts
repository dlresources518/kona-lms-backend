...
export default class AdminController {
      ...
      form.parse(req, async (err, fields, files: any) => {
        ...
        if (checkBatch) {
          ...
          if (isCurriculumPresent && isTopicPresent) {
            ...
            if (fields.isDelete === "true") {
            ...
            } else if (fields.assignmentId !== "undefined") {
              checkBatch.curriculum[curriculumIndex].topics[
                topicIndex
              ].assignments.forEach((r: any) => {
                if (r._id == fields.assignmentId) {
                  r.title = fields.title;
                  r.submitOn = fields.submitOn;
                  r.instructions = fields.instructions;
                  r.instructorMailId = fields.instructorMailId;
                  r.solution = {
              ...
              checkBatch.curriculum[curriculumIndex].topics[
                ...
                title: fields.title,
                submitOn: fields.submitOn,
                instructions: fields.instructions,
                instructorMailId: fields.instructorMailId,
                solution: {
                  content: fields.content,
                  fileLink:
                    awsResponse && awsResponse.Location
  ...
  public allRegisteredUsers = async (req: Request, res: Response) => {
    try {
  ...
  public updateCoursesToStudent = async (req: Request, res: Response) => {
    try {
      await Promise.all(
